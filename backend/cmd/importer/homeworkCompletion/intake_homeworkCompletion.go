...
func main() {
	...
	for _, doc := range studentDocs {
		...
		// Read data from the student's 'Homework Completion' sheet
		homeworkDataRows, err := fetchHomeworkCompletionSheetData(sheetService, sheetID)
		if err != nil {
			log.Printf("Error reading homework completion sheet for %s: %v", studentName, err)
			continue
		}
		if len(homeworkDataRows) == 0 {
			log.Printf("No data found in homework completion sheet for %s", studentName)
			continue
		}

		// Process and store each row of homework data into Firestore
		for _, homeworkData := range homeworkDataRows {
			dateStr, ok := homeworkData["date"].(string)
			if !ok || dateStr == "" {
				continue
			}
			...
			// Create or update the document in 'Homework Completion' subcollection
			homeworkCollection := doc.Ref.Collection("Homework Completion")
			homeworkDocRef := homeworkCollection.Doc(safeDateStr)

			_, err := homeworkDocRef.Set(ctx, homeworkData)
			if err != nil {
				log.Printf("Failed to write homework data for student %s, date %s: %v", studentName, dateStr, err)
				continue
			}
			log.Printf("Successfully wrote homework data for student %s, date %s", studentName, dateStr)
		}
...
// Helper function to read data from a student's 'Homework Completion' sheet in Google Sheets
func fetchHomeworkCompletionSheetData(sheetService *sheets.Service, sheetID string) ([]map[string]interface{}, error) {
	// Read data from the 'Homework Completion' sheet
	readRange := "Homework Completion!A:B"
	resp, err := sheetService.Spreadsheets.Values.Get(sheetID, readRange).Do()
	if err != nil {
		log.Printf("Error reading homework completion data from sheet ID: %s, Range: %s", sheetID, readRange)
		return nil, err
	}

	// Process each row
	var homeworkData []map[string]interface{}
	for _, row := range resp.Values {
		...
		percentageStr := fmt.Sprintf("%v", row[1]) // Handle numbers and strings
		if percentageStr == "" {
			continue
		}

		// Handle 'N/A' or other non-numeric values
		if strings.EqualFold(percentageStr, "N/A") {
			log.Printf("Skipping 'N/A' percentage value for date '%s'", dateStr)
			continue
		}

		// Remove '%' symbol if present
		percentageStr = strings.TrimSpace(strings.TrimSuffix(percentageStr, "%"))

		// Parse percentage as float
		percentage, err := strconv.ParseFloat(percentageStr, 64)
		if err != nil {
			log.Printf("Invalid percentage value '%s' for date '%s'", percentageStr, dateStr)
			continue
		}
		...
		homeworkData = append(homeworkData, data)
	}
